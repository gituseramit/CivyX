...
import { ComplaintService, Complaint, TimelineEntry } from '../../core/services/complaint.service';
        ...
        @if (!loading() && complaints().length === 0) {
          <div class="institutional-glass p-16 rounded-2xl text-center border-dashed border-white/10">
            <div class="w-20 h-20 rounded-full bg-white/[0.03] flex items-center justify-center mx-auto mb-8">
               <mat-icon class="!text-4xl text-white/10">inventory_2</mat-icon>
            </div>
              ...
              <div class="p-8 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-6"
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex flex-wrap gap-2 mt-4">
                    ...
                    }">{{ c.status === 'in_progress' ? 'Rectification Phase' : c.status === 'acknowledged' ? 'Intelligence Confirmed' : c.status }}</span>
                    @if (c.ai_classified) {
...
export class MyComplaintsComponent implements OnInit {
  ...
  toggle(id: string) {
    ...
    this.complaintSvc.getById(id).subscribe({
      next: (res) => { this.timeline.set(res.timeline); this.timelineLoading.set(false); },
